   ...
   <div class="wrapper">
      ...
      <div class="container">
         ...
         <div class="px-3 py-3 pr-3 mx-auto float-right">
            <button class="btn btn-danger" @click="order">Đặt hàng ngay</button>
         </div>
         <div class="px-3 py-3 pr-3 mx-auto float-right">
...
export default {
   ...
   methods: {
      ...
      order() {
         if (localStorage.getItem("localProductCart")) {
            localStorage.removeItem("localProductCart");
            alert("Bạn đã đặt hàng thành công");
            window.location.reload();
         } else {
            alert("Bạn chưa thêm sản phẩm nào vào giỏ hàng");
         }
      },
   },
   mounted() {
